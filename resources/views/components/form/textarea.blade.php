...
<div data-role="form-textarea"
    ...
    @isset($label)
        <label for="textarea-{{ $name }}" class="w-full font-semibold">{{ $label }}</label>
    @endisset
    <div class="flex items-center justify-center w-full gap-2 px-2 py-2 text-base border-2 border-black rounded-lg">
        @isset($icon)
